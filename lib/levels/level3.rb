class Level3 < Level
  ...
  def initialize
    super("Stage 3, Storage", "Clear the storage area of hostiles.")
    ...
    @texts = [
      ...
      TextTyper.new(50, 625, "ENV: WIND #{@environment.wind_direction.to_s[0..0].upcase}, 3 km/h#{", #{@environment.weather.to_s.upcase}" if @environment.weather != :normal}")
    ]
