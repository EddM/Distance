class Level4 < Level
  ...
  def initialize
    super("Stage 4, Barracks", "Take out the guards around the barracks.")
    @environment = Environment.new(16, :east)
    ...
    @texts = [
      ...
      TextTyper.new(50, 625, "ENV: WIND #{@environment.wind_direction.to_s[0..0].upcase}, 16 km/h#{", #{@environment.weather.to_s.upcase}" if @environment.weather != :normal}")
    ]
